...
class _registroScreenState extends State<registroScreen> {
 ...
 Future<void> _registro(String nombre, String contrasena, String correo) async {
  //se acaba de agregar para validar la contraseña 
  if (contrasena.length < 8 || !contrasena.contains(new RegExp(r'[!@#$%^&*(),._":]'))) {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("Error"),
            content: Text("La contraseña es muy corta o no contiene al menos 1 carácter especial"),
            actions: <Widget>[
              TextButton(
                child: Text("OK"),
                onPressed: () {
  ...
  //esto se agrego verifica si el usuario ya existe
